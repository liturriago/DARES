...
class DARESTrainer(BaseTrainer):
    ...
    _metric_keys: tuple[str, ...] = _METRIC_KEYS
    ...
    def __init__(
        ...
        self.criterion = self._build_criterion().to(device)

        self.ref_params = list(self.model.backbone.reference_params)
        self.optimizer = self._make_optimizer(self.model.parameters())

    def _criterion_kwargs(self) -> dict[str, Any]:
        """Shared constructor kwargs for the DARES criterion family."""
        config = self.config
        return {
            ...
            "gamma": getattr(config, "repulsion_gamma", 0.5),
            ...
            "normalize_features": getattr(config, "normalize_features", False),
            "weight_cross": getattr(config, "weight_cross", False),
            "sigma_min": getattr(config, "sigma_min", 1e-3),
            "sigma_max": getattr(config, "sigma_max", 1e3),
        }

    def _build_criterion(self) -> DARESLoss:
        """Instantiates the criterion; overridden by the v2 engine."""
        return DARESLoss(**self._criterion_kwargs())
    ...
    def _train_warmup_epoch(self) -> dict[str, float]:
        ...
        acc: dict[str, float] = {k: 0.0 for k in self._metric_keys}
        ...
        for _ in pbar:
            ...
            for key in self._metric_keys:
                if key == "loss_total":
                    ...
                    value = float(parts[key].detach().item())
                acc[key] += 0.0 if value != value else value
